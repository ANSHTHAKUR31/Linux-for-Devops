...
- Agar DevOps aur Linux exist nahi karte, to bhi ban jaayenge , Error nahi aata

D.  ``` mkdir -v ```
- Verbose mode (batata hai kya create hua)
- ``` mkdir -v test ```
### Output:
...
## 5.    Delete directory    
- A. ```rmdir ```       => for deleting the empty directory
- B. ```rmdir -r  ```   => for deleting the directory with its content 
- C. ```rmdir -rf ```   => with content + forcefully 
- D. ```rmdir -ri  ```  => with content + will ask the permission to delete
...
- A. ```cat file.txt``` => will display the content of the files 
- B. ```cat file1.txt  file2.txt ``` => will display both files content 
- C. ```cat > file.txt ```  => will create the new file ( control +d)to save it.
- D. ```cat >> file.txt ```=> adding new content to existing file
- E. ```cat -n file.txt```  => will display the content with  line number 
- F. ```Zcat file.gz ``` => will display the content of that compressed file
...
- A. ```head file.txt ```  => will display 10 line of the file 
- B. ```head -n 5 file.txt ```  => will display 5 lines only 
- C. ```head -c 20 file.txt```  => will display the 20 byte only
...
- A. ```tail file.txt```  => will display the last 10 lines of the files
- B. ```tail -n 5 file.txt ```   => will display the last 5 lines only 
- C. ```tail -c 10 file.txt ``` => will display the 10 byte of file
- D. ```tail -f file.txt ``` => whenever any new line added in the file it will show on the terminal
...
* A. Simple output save + display
- ```ls | tee files.txt```

### Output  terminal  
- ```A1 backup1.txt 	file4.txt 	newfolder```
### Output  files.txt   
- ```A1 backup1.txt 	file4.txt 	newfolder```
